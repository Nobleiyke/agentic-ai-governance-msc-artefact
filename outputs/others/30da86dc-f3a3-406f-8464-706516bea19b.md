...
- **Date**: 2026-04-09
- **Status note (important)**: The **CVE-bench execution log/results are not yet available in this repository**, so this report includes:
  - a complete write-up of the **statistical analysis outputs already produced** in `outputs/`
  - a complete write-up of the **qualitative (NVivo) deductive coding design** and how to compute “intersection density”
  - a clearly marked **placeholder/results plan** for CVE-bench logs once runs are completed
...
- **CVE-bench integration plan (pending)**: CVE-bench run logs are not yet in this repo, so I explain exactly how they will be added once runs are complete.
...
- **Not yet present** in this repo (no run output files found).
  - This report includes a “CVE-bench integration” section describing exactly how results will be incorporated once available.
...
- Clearly marked what is **complete now** vs what is **pending** (CVE-bench logs/results).
...
- **Say**: “My results show structured performance regimes, strong task effects, and clear intervention-related gaps. Next, I will finalise CVE-bench runs (PowerShell + Docker-in-Docker) and attach the run logs so benchmark traces can be mapped into the same ARC categories and triangulated with NVivo and statistics.”
...
## CVE-bench integration (planned; pending logs/results)

### Current limitation
This repo contains CVE-bench code (`src/cvebench/cvebench.py`) but **does not yet contain CVE-bench run output logs** (e.g., JSONL/JSON summaries of attempts, tool traces, success/failure per CVE).

### How CVE-bench results will be included once available
When CVE-bench runs are completed, the report should add:

- **Run configuration**: model, agent settings, tool permissions, number of tasks.
- **Outcome table**: success/partial/fail counts and rates.
- **ARC mapping of traces**:
  - capability evidence (web/search, tool use, code execution, file/data ops),
  - failure modes (prompt injection, tool malfunction, agent failure),
  - hazards (security, integrity, privacy, infrastructure).
- **Triangulation**:
  - show 2–3 representative CVE-bench traces as “worked examples”
  - demonstrate that the same high-density intersections appear in NVivo matrices and in benchmark traces.
...
- **CVE-bench is the missing empirical trace layer**: once available, it will let you demonstrate concrete capability→failure→hazard pathways with benchmark evidence.
